...
while diff > tol
    ...
    for ik = 1 : nk
        ...
        for iaalpha = 1 : naalpha
            ...
            for iz = 1 : nz               
                ...
                if (mod(iter_Accelerator,10)==0 || iter_Accelerator ==1)
                    ...
                    capitalNext = policy.k(ik,iaalpha,iz);
                end
    ...
    if (mod(iter_Accelerator,10)==0 || iter_Accelerator ==1)
        fprintf(' Iteration = %d, Sup Diff = %2.8f\n', iter_Accelerator, diff); 
    end
    iter_Accelerator = iter_Accelerator + 1;
end
time_Accelerator = toc;
...
for i = 1 : num_plots
    ...
    ylabel(ylabel_names{i},'FontSize',7)
    ylim(ylims{i})
    saveas(gcf,['..' filesep 'Output' filesep 'Q5_Accelerator_' file_label{i} '.png']);
    close(gcf)
end
...
for i = 1 : length(shock_variable)
    ...
    saveas(gcf,['..' filesep 'Output' filesep 'Q5_Accelerator_IRF_' shock_variable{i} '.png']);
    close(gcf)
end

%% Compute Euler Equation Errors
[euler_error_Accelerator,max_error_Accelerator] = euler_error_VFI(aalpha,bbeta,ggamma,ddelta,ppsi, ...
    eeta,V,policy.c,policy.l,policy.k,z,grid.capital,naalpha,nz,nk);

%% Save results
save(['..' filesep 'Output' filesep 'Q5_Accelerator.mat'])